...
const path = process.env.CREATE_DIRECTORATE_PATH;

const randomIndex = GenerateRandom.number();
const name = `depar${randomIndex}`;
// const function_id = `CORE${randomIndex}`;

let api, directorateId;
...
const afterEachTest = async () => {
    if (directorateId != null) {
        const deleteDirectoratePath = process.env.DELETE_DIRECTORATE_PATH + directorateId;
        await api.delete_Directorate(deleteDirectoratePath, headers);
    } else {
        console.log('Function Directorate is Not Exist');
    }
}

describe('Create Function Directorate', () => {
    beforeEach(beforeEachTest);
    test.only('with valid data', async () => {
        
        const response = await api.create_directorate(path, headers, body);
    ...
    test('with existing data', async () => {
        body.update('name', 'directorate-test-automation-1');
        const response = await api.create_func_directorate(path, headers, body);
        expect(response.statusCode).equal(422);
        expect(response.body.status).equal('ERROR');
        expect(response.body.message).equal('Function Directorate Already Exist');
    }),
    
    test('without input Directorate data', async () => {
        body.update('name', '');
        const response = await api.create_func_directorate(path, headers, body);
        expect(response.statusCode).equal(422);
        expect(response.body.status).equal('ERROR');
