...
const beforeEachTest = async () => {
    ...
    if (responseAdd.statusCode == 200) {
        BudgetId = responseAdd.body.data.id;
    }
}

const afterEachTest = async () => {
    console.log('hai sayangg');
    const getBudgetPath = process.env.GET_DETAIL_FUNC_BUDGET_PATH + budgetId;
    const responseGetID = await api.get_detail(getBudgetPath, headers);
...
describe('Delete Budget', () => {
    ...
    test.only('with valid id data', async () => {
        ...
        const response = await api.delete_func_budget(deleteBudgetPath, headers);
        expect(response.statusCode).equal(200);
        expect(response.body.status).equal('OK');
        expect(response.body.message).equal('Delete Function Budget Succes');
