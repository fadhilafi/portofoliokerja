...
describe('Detail stack', () => {
    ...
    test('with valid id data', async () => {
        ...
        const response = await api.get_detail(getStackPath, headers);
        expect(response.statusCode).equal(200);
        expect(response.body.status).equal('SUCCESS');
        expect(response.body.message).equal('Get Detail Base Stack By ID Success');
    ...
    test('with unexisting id data', async () => {
        const getStackPath = process.env.DETAIL_STACK_PATH + stackId + "xx0";
        const response = await api.get_detail(getStackPath, headers);
    ...
    test('without input id data', async () => {
        const getStackPath = process.env.DETAIL_STACK_PATH + ":id";
        const response = await api.get_detail(getStackPath, headers);
        expect(response.statusCode).equal(404);
        expect(response.body.status).equal('ERROR');
