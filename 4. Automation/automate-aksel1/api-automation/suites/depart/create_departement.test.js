...
const path = process.env.CREATE_DEPARTEMENT_PATH;
...
const afterEachTest = async () => {
    if (departementId != null) {
        const deleteDepartementPath = process.env.DELETE_DEPARTEMENT_PATH + departementId;
        await api.delete_Departement(deleteDepartementPath, headers);
    } else {
        console.log('Function Departement is Not Exist');
    }
}

describe('Create Function Departement', () => {
    beforeEach(beforeEachTest);
    test.only('with valid data', async () => {
        
        const response = await api.create_departement(path, headers, body);
    ...
    test('with existing data', async () => {
        body.update('name', 'departement-test-automation-1');
        const response = await api.create_func_departement(path, headers, body);
        expect(response.statusCode).equal(422);
        expect(response.body.status).equal('ERROR');
        expect(response.body.message).equal('Function Departement Already Exist');
    }),
    
    test('without input Departement data', async () => {
        body.update('name', '');
        const response = await api.create_func_departement(path, headers, body);
        expect(response.statusCode).equal(422);
        expect(response.body.status).equal('ERROR');
