...
// describe('Detail position', () => {
//     beforeEach(beforeEachTest);
    
//     test('with valid id data', async () => {
//         const getPositionPath = process.env.DETAIL_POSITION_PATH + positionId;
//         const response = await api.get_detail(getPositionPath, headers);
//         expect(response.statusCode).equal(200);
//         expect(response.body.status).equal('SUCCESS');
//         expect(response.body.message).equal('Get Detail Base Position By ID Success');
...
//         const getPositionPath = process.env.DETAIL_POSITION_PATH + positionId + "xx0";
//         const response = await api.get_detail(getPositionPath, headers);
...
//         const getPositionPath = process.env.DETAIL_POSITION_PATH + ":id";
//         const response = await api.get_detail(getPositionPath, headers);
//         expect(response.statusCode).equal(404);
//         expect(response.body.status).equal('ERROR');
