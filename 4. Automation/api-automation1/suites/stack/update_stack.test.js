...
describe('Update stack', () => {
    ...
    test('with valid data', async () => {
        ...
        const updateStackPath = process.env.UPDATE_STACK_PATH + stackId;
        const response = await api.update_stack(updateStackPath, headers, body);
        expect(response.statusCode).equal(200);
        expect(response.body.status).equal('SUCCESS');
        expect(response.body.message).equal('Update Base Stack Success');
